...
export const FaceSwap: React.FC<FaceSwapProps> = ({ 
  ...
  const handleSwap = async () => {
    if (!selectedInfluencer || !targetImage) {
      setError("Please select an influencer and upload a target image.");
      return;
    }
    ...
    // Construct prompt for "Swap"
    const swapPrompt = `Face Swap Operation. Replace the face of the person in the second image with the face of ${selectedInfluencer.name} ${physicalContext} (from the first image). ${selectedInfluencer.description || ''}. Keep the original pose, lighting, clothing, and background of the second image exactly as is. Ensure high photorealism and seamless blending.`;
    ...
    try {
      ...
      if (result.imageUrl) {
        ...
        setError("Failed to generate swap image.");
      }
    } catch (err: any) {
      setError(err.message || "Face swap failed.");
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <section className="lg:col-span-4 space-y-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Select AI Character (Source)</label>
              ...
              <div>
                 <label className="block text-sm font-medium text-slate-300 mb-2">Add Target Files</label>
                 <div 
              ...
              >
                {isProcessing ? "Swapping..." : "Swap Face"}
              </Button>
        ...
        <section className="lg:col-span-8">
           <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl min-h-[600px]">
              <h2 className="text-lg font-semibold text-white mb-6">Face Swap Gallery</h2>
              ...
              {swapGallery.length === 0 ? (
                 <div className="flex flex-col items-center justify-center h-[400px] text-slate-500 border border-dashed border-slate-700/50 rounded-xl">
                    <p>Processed face swaps will appear here</p>
                 </div>
              ) : (
